...
public void EnviarNitificaciones (MouseRidersGenNHibernate.EN.MouseRiders.PermisoEN permiso, MouseRidersGenNHibernate.EN.MouseRiders.RecompensaEN oidRecompensa, string asunto, string texto, string adjunto)
{
        ...
        try
        {
                ...
                for (int i = 0; i < users.Count; i++) {
                        if (permiso == null && oidRecompensa == null) { //la lista acotada contiene todos los usuarios
                                acotao.Add (users [i].Email);
                        }

                        else if (permiso == null && oidRecompensa != null) { //la lista acotada contiene los usuarios con la recompensa pasado por parametro
                                if (users [i].Obtiene.Contains (oidRecompensa)) {
                                        acotao.Add (users [i].Email);
                                }
                        }

                        else if (permiso != null && oidRecompensa == null) { //la lista acotada contiene los usuarios con el permiso pasado por parametro
                                if (users [i].Tiene.Contains (permiso)) {
                                        acotao.Add (users [i].Email);
                                }
                        }

                        else{ // la lista acotada tiene en cuenta tanto el permiso como la recompensa
                                if (users [i].Obtiene.Contains (oidRecompensa)) {
                                        if (users [i].Tiene.Contains (permiso)) {
                                                acotao.Add (users [i].Email);
                                        }
